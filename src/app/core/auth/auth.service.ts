...
export class AuthService {
    ...
    signIn(credentials: { email: string; password: string }): Observable<any> {
        ...
        return this._httpClient.post(environment.apiUrl + '/auth/login', credentials).pipe(
            switchMap((response: any) => {
                ...
                // Store the user on the user service and local storage
                this._userService.user = response.user;
                localStorage.setItem('schoolos_user', JSON.stringify(response.user));
    ...
    signInUsingToken(): Observable<any> {
        ...
        return this._httpClient.get(environment.apiUrl + '/auth/me').pipe(
            catchError(() =>
                // Return false
                of(false)
            ),
